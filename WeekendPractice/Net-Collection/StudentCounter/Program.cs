...
namespace StudentCounter
{
    class Program
    {
        ...
        public static List<string> ListOfNames(List<Dictionary<string, object>> userInput)
        {
            List<string> namesWithMoreCandies = new List<string>() { };

            foreach (var item in userInput)
            {
                int candies = userInput.GetValueOrDefault(item)
                if (candies > 4)
                {
                    namesWithMoreCandies.Add(item["name"]);
                }
